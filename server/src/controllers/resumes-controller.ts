...
const postResume = async (req: Request, res: Response) => {
    ...
    const resumeData = { name, summary, experiences };
		const insertResult = await database.collection("resumes").insertOne(resumeData)
    const newResume = await database.collection("resumes").findOne({ _id: insertResult.insertedId })
...
const editResume = async (req: Request, res: Response) => {
    ...
    const updateResult = await database.collection("resumes").updateOne({ _id: new ObjectId(resumeId) }, { $set: updatedItem });
    const updatedResume = await database.collection("resumes").findOne({_id: new ObjectId(resumeId)})
